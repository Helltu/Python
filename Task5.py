jewelry = {'браслет': ['золото', 15, 3], 'кольцо': ['серебро', 5, 32], 'ожерелье': ['бронза', 1, 15], 'перстень': ['алмаз', 500, 2],'браслет изумредный': ['изумруд', 200, 10], 'кольцо серебряное': ['серебро', 50, 100],'браслет уникальный': ['метеорит', 100, 1], 'кольцо новое': ['рубин', 5, 320]}
cont_loop = True
while cont_loop:
    ...
    if key == '1':
        ...
        if(not(amount.isdigit())):
            ...
            if not(got):
                ...
                print('\tОбщая стоимость купленных изделий: ' + str(got[1]*amount))
                print('\tКоличество данных изделий оставшихся в магазине: ' + str(got[2]-amount))
